...
- **Services**：`install-preview-service.zsh` 安装 `~/Library/Services/YulanCopyPreviewPage.workflow`（`CFBundle*`、`NSSendTypes`、`com.apple.Automator.text`）；预览内需先划选少量文字再出「服务」菜单。
